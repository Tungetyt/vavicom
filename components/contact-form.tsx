...
export default function ContactForm() {
  ...
  const [sentState, setSentState] = useState<"error" | "success" | "loading" | null>(null);
  ...
  return (
    <div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-white dark:bg-black">
      <h2 className="font-bold text-xl text-neutral-800 dark:text-neutral-200">
        Formularz kontaktowy do {contactEmail} <span className="whitespace-nowrap">({contactPhone})</span>
      </h2>
      ...
      <form ref={ref} className="mt-8" onSubmit={handleSubmit}>
        ...
        {sentState === "success" ? <p className="text-green-700">Wiadomość została wysłana! Dziękujemy!</p> :
          <button
        ...
        {sentState === "error" && <p className="text-destructive">Nastąpił błąd, wiadomość nie została wysłana. Przepraszamy.</p>}
      </form>
    </div>
  );
}
